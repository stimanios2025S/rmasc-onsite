...
import {
  ...
  LayoutDashboard, Settings, LogOut, Search, Sun, ArrowUpRight, CheckCheck,
} from 'lucide-react';
...
const PRIORITE_COULEUR: Record<string, string> = {
  ...
  moyenne: 'bg-amber-400 text-stone-800', basse: 'bg-slate-100 text-slate-500',
};
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    if (!estConnecte()) { router.push('/login'); return; }
    loadAll();
  ...
  async function handleApprouver(demandeId: string) {
    setActionLoading(demandeId);
    try { await approuverDemande(demandeId); await loadAll(); }
  ...
  async function handleRefuser(demandeId: string) {
    ...
    setActionLoading(demandeId);
    try { await refuserDemande(demandeId); await loadAll(); }
    catch (e: any) { alert(e.message); }
    setActionLoading(null);
  }

  const equipeTypes = useMemo(() => {
    const groups: Record<string, EquipeData[]> = {};
  ...
  const totalMissions = stats?.missionsTotal ?? 0;
  const completedMissions = totalMissions > 0 ? Math.round((stats?.chantiersTotal ?? 0) / (totalMissions || 1) * 100) : 0;

  return (
    <div className="min-h-screen bg-gradient-to-br from-[#FFF5EC] via-[#FDE8DF] to-[#F3E8FF] flex">
      {/* ═══════════════════ SIDEBAR ═══════════════════════════════ */}
      <aside className="hidden lg:flex flex-col w-64 bg-white/60 backdrop-blur-xl border-r border-white/40 p-5 shrink-0">
        ...
        <button
          ...
          className="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm text-stone-400 hover:text-red-500 hover:bg-red-50 transition-all"
        >
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        {/* ─── TOP BAR ───────────────────────────────────────────── */}
        <header className="bg-white/70 backdrop-blur-xl border-b border-white/30 px-6 lg:px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            {/* Mobile menu icon */}
            <button className="lg:hidden p-2 rounded-xl bg-stone-100 text-stone-500">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 12h18M3 6h18M3 18h18"/></svg>
            </button>
            {/* Time filter pills */}
            <div className="hidden md:flex items-center bg-white rounded-2xl shadow-sm border border-stone-100 p-1">
              {["Aujourd'hui", 'Cette semaine', 'Ce mois'].map((t) => (
                <button
                  key={t}
                  onClick={() => setFiltreTemps(t)}
                  className={`px-4 py-1.5 rounded-xl text-xs font-medium transition-all ${
                    filtreTemps === t ? 'bg-stone-800 text-white shadow-sm' : 'text-stone-400 hover:text-stone-600'
                  }`}
                >
                  {t}
                </button>
          ...
          <div className="flex items-center gap-4">
            {/* Search */}
            <div className="hidden sm:flex items-center gap-2 bg-stone-50 rounded-xl px-3 py-1.5 border border-stone-100">
              <Search size={14} className="text-stone-300" />
              <input placeholder="Rechercher..." className="bg-transparent text-xs text-stone-500 outline-none w-28 placeholder:text-stone-300" />
              <span className="text-[10px] text-stone-300 bg-stone-100 px-1.5 py-0.5 rounded">/</span>
            </div>
            {/* Notification */}
            <button className="relative p-2 rounded-xl hover:bg-stone-50 transition-colors">
              <Bell size={18} className="text-stone-400" />
              {stats && stats.demandesEnAttente > 0 && (
            ...
            <div className="flex items-center gap-2.5 pl-4 border-l border-stone-200">
              ...
              <div className="hidden sm:block">
                <p className="text-xs font-semibold text-stone-700">El Ghani</p>
            ...
            <div className="mb-8 bg-white/90 backdrop-blur-md rounded-3xl border border-rose-100 shadow-lg overflow-hidden">
              <div className="bg-gradient-to-r from-rose-50 to-orange-50 px-6 py-4 flex items-center justify-between border-b border-rose-100">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="font-bold text-stone-800">Commandes Factory à Valider</h2>
                    <p className="text-xs text-stone-400">{demandes.length} en attente d&apos;approbation</p>
                  </div>
                </div>
                  ...
                  <div key={d.id} className="px-6 py-4 flex items-center justify-between hover:bg-rose-50/30 transition-colors">
                    ...
                    <div className="flex items-center gap-2 ml-4">
                      <button onClick={() => handleApprouver(d.id)} disabled={actionLoading === d.id}
                        className="flex items-center gap-1.5 bg-emerald-500 text-white px-4 py-2 rounded-xl text-sm font-semibold hover:bg-emerald-600 disabled:opacity-50 transition-all shadow-sm"
                      >
                        {actionLoading === d.id ? <Loader2 size={15} className="animate-spin" /> : <CheckCheck size={15} />}
                        Valider
                      </button>
                      <button onClick={() => handleRefuser(d.id)} disabled={actionLoading === d.id}
                        className="flex items-center gap-1.5 bg-white text-rose-500 px-4 py-2 rounded-xl text-sm font-semibold border border-rose-200 hover:bg-rose-50 disabled:opacity-50 transition-all"
                      >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div className="bg-white/90 backdrop-blur-md rounded-3xl border border-stone-100 shadow-sm p-5 flex flex-col items-center justify-center">
              <div className="relative w-24 h-12 overflow-hidden mb-2">
                ...
                <div className="absolute inset-0 rounded-t-full border-8 border-transparent border-t-emerald-400 border-r-emerald-400"
                  style={{ rotate: `${completedMissions * 1.8 - 180}deg` }}
                />
              </div>
              <p className="text-2xl font-bold text-stone-800">{completedMissions}%</p>
                  ...
                  return (
                    <div key={type}>
                      <div className="flex items-center gap-2 text-xs font-semibold text-stone-500 mb-3">
                        <Icon size={14} />
                        {type === 'mecanique' ? 'Équipes Mécaniques' : type === 'electrique' ? 'Équipes Électriques' : 'Équipes Vérification'}
                        <span className="text-stone-300">({eqs.length})</span>
                      </div>
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                        {eqs.map((eq) => (
                          <div key={eq.id} className={`rounded-2xl p-4 border shadow-sm transition-all ${
                            eq.statut_equipe === 'EN_REPOS' ? 'border-amber-200 bg-amber-50/30' :
                            eq.statut_equipe === 'EN_MISSION' ? 'border-indigo-200 bg-indigo-50/30' :
                            ...
                            {eq.statut_equipe === 'EN_REPOS' && eq.jours_repos_restants > 0 && (
                              <p className="text-[11px] text-amber-600 font-medium">⏳ {eq.jours_repos_restants}j restants</p>
                            )}
                            {eq.statut_equipe === 'EN_MISSION' && (
                              <p className="text-[11px] text-indigo-600 font-medium">🔧 {eq.missions} mission{eq.missions > 1 ? 's' : ''}</p>
                            )}
                            {eq.statut_equipe === 'DISPONIBLE' && (
                              <p className="text-[11px] text-emerald-600 font-medium">✅ Prêt</p>
                            )}
                          </div>
                        ))}
                      </div>
        ...
        <footer className="text-center pb-4">
          <p className="text-[10px] text-stone-300">RMASC OnSite v1.0 — Plateforme de gestion des opérations de terrain</p>
        </footer>
      </div>
    </div>
  );
...
/* ─── KPI CARD ──────────────────────────────────────────────────────── */
function KpiCard({ titre, valeur, couleur, icon, badge }: {
  titre: string; valeur: number; couleur: string; icon: React.ReactNode; badge?: boolean;
}) {
  const colorMap: Record<string, string> = {
    emerald: 'bg-emerald-50 text-emerald-500', rose: 'bg-rose-50 text-rose-500',
    indigo: 'bg-indigo-50 text-indigo-500', amber: 'bg-amber-50 text-amber-500',
  };
  return (
    <div className="relative bg-white/90 backdrop-blur-md rounded-3xl border border-stone-100 shadow-sm p-5 flex items-start gap-4 hover:shadow-md transition-all">
      ...
      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${colorMap[couleur]}`}>{icon}</div>
      <div>
        <p className="text-[11px] font-semibold text-stone-400 uppercase tracking-wider mb-0.5">{titre}</p>
        <span className="text-2xl font-bold text-stone-800">{valeur}</span>
      </div>
    </div>
  );
}
...
function MapWidget({ chantiers }: { chantiers: ChantierData[] }) {
  const dotColor = (s: string) =>
    s === 'en_cours' ? 'bg-emerald-400' : s === 'bloque' ? 'bg-rose-400' :
    s === 'termine' || s === 'reception_officielle' ? 'bg-stone-300' : 'bg-indigo-400';
  return (
    <div className="bg-white/90 backdrop-blur-md rounded-3xl border border-stone-100 shadow-sm overflow-hidden">
      ...
      <div className="p-6">
        {chantiers.length === 0 ? (
          <p className="text-center text-stone-400 py-12 text-sm">Aucun chantier actif.</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[420px] overflow-y-auto">
            {chantiers.map((c) => (
              <div key={c.id} className="flex items-start gap-3 p-3.5 rounded-2xl border border-stone-100 hover:bg-stone-50 transition-colors cursor-pointer">
                <div className={`w-3 h-3 mt-1.5 rounded-full shrink-0 ${dotColor(c.statut)}`} />
                <div className="flex-1 min-w-0">
    ...
    <div className="bg-white/90 backdrop-blur-md rounded-3xl border border-stone-100 shadow-sm h-full">
      <div className="px-5 py-4 border-b border-stone-100 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-stone-800">Blocages & Incidents</h3>
        </div>
        <span className="text-[11px] text-stone-400">
          {blocages.length + pointages.length} récents
        </span>
      </div>
      <div className="divide-y divide-stone-50 max-h-[520px] overflow-y-auto">
        {blocages.length === 0 && pointages.length === 0 ? (
          <p className="py-10 text-center text-stone-400 text-sm">Tout est sous contrôle ✓</p>
        ) : (
          <>
            {blocages.slice(0, 6).map((inc, i) => (
              <div key={`b-${i}`} className="px-5 py-3.5 hover:bg-rose-50/20 transition-colors">
                <div className="flex items-start justify-between gap-2">
                  <p className="text-sm font-semibold text-stone-700 leading-snug">{inc.message}</p>
                  <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded whitespace-nowrap ${PRIORITE_COULEUR[inc.priorite] || 'bg-stone-100 text-stone-500'}`}>
            ...
            {pointages.slice(0, 4).map((inc, i) => (
              <div key={`p-${i}`} className="px-5 py-3.5 hover:bg-stone-50 transition-colors">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm text-stone-600">{inc.message}</p>
                </div>
