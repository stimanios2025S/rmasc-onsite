...
// ─── Rapport de vérification (HTML auto-printable) ──────────────────
app.get('/api/mission/:id/rapport', async (req, res) => {
  try {
    const missionId = req.params.id;

    // Fetch mission + chantier + checklist + team info
...
<body>
  <div class="header">
    ...
    <div class="subtitle">RMASC OnSite — Diagnostic d'installation</div>
  </div>

  <div class="meta">
    <div class="meta-item"><span class="label">Chantier</span><span class="value">${m.nom_chantier}</span></div>
    <div class="meta-item"><span class="label">Référence ERP</span><span class="value">${m.reference_erp}</span></div>
    ...
    <div class="meta-item"><span class="label">Équipe Vérification</span><span class="value">${m.equipe_nom}</span></div>
  ...
  <div class="score-banner" style="background: ${verdictColor}15; border: 2px solid ${verdictColor};">
    ...
    <div class="stats">${passedSteps} conforme${passedSteps > 1 ? 's' : ''} sur ${totalSteps} points vérifiés${failedSteps > 0 ? ` — ${failedSteps} non conforme${failedSteps > 1 ? 's' : ''}` : ''}</div>
  </div>

  <div class="section">
    <div class="section-title">Points de vérification (${totalSteps})</div>
  ...
  <div class="footer">
    Rapport généré automatiquement par RMASC OnSite — ${now}<br>
    Ce document est un diagnostic interne. Validé par l'équipe de vérification.
  </div>
  ...
  <div class="no-print" style="text-align:center; margin-top:24px;">
    ...
    res.status(500).send('Erreur lors de la génération du rapport: ' + err.message);
  }
});
