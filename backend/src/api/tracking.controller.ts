        ...
        if (equipeNextRes.rows.length > 0) {
          ...
          try {
            ...
            try {
              ...
              logger.info('Checklist fallback créée', { missionNextId, phase: nextPhase });
            } catch (fbErr: any) {
              logger.error('Échec total création checklist fallback', { erreur: fbErr.message, missionNextId });
            }
          }
        } else {
          const result = await pool.query(
